...
function wpgrade_callback_portfolio_posts_per_page( $query ) {
	...
	if ( !empty($query->query_vars['post_type']) && is_archive()) {
		...
		if ( $query->query_vars['post_type'] == 'lens_portfolio' ) {
			if (is_numeric( wpgrade::option('portfolio_projects_per_page') ))
				$query->query_vars['posts_per_page'] = wpgrade::option('portfolio_projects_per_page');
		} elseif ($query->query_vars['post_type'] == 'lens_gallery') {
			if (is_numeric( wpgrade::option('galleries_per_page') ))
				$query->query_vars['posts_per_page'] = wpgrade::option('galleries_per_page');
		}
	}
	return $query;
}
