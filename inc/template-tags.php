...
function wpgrade_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'lens' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'lens' ), ' ' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<article id="comment-<?php comment_ID(); ?>">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s', 'lens' ), sprintf( '<span class="fn">%s</span>', get_comment_author_link() ) ); ?>
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'lens' ); ?></em>
					<br />
				<?php endif; ?>
...
function wpgrade_posted_on() {
	printf( __( '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a>', 'lens' ),
